create table USER (
  USER_ID BIGINT(19) auto_increment,
  AUDIT_CD TIMESTAMP,
  AUDIT_RD TIMESTAMP,
  AUDIT_MD TIMESTAMP,
  LOGIN varchar(256),
  FIRSTNAME varchar(256),
  LASTNAME varchar(256),
  PASSWORD varchar(256),
  EMAIL varchar(256),
  GENERALPOINTSQUANTITY BIGINT(19),
  ENABLED boolean
);

create table FRIENDSHIP (
  USER_ID BIGINT(19),
  FRIEND_ID BIGINT(19)
);

create table USER_ROLE (
  ROLE_ID BIGINT(19) auto_increment,
  USER_ID BIGINT(19),
  ROLE varchar(255),
  FOREIGN KEY (USER_ID) REFERENCES USER(USER_ID)
);

create table USER_GROUPS_MEMBERSHIP (
  USER_ID BIGINT(19),
  GROUP_ID BIGINT(19)
);

create table CHALLENGE (
  CHALLENGE_ID    BIGINT(19) auto_increment,
  AUDIT_CD        TIMESTAMP,
  AUDIT_RD        TIMESTAMP,
  AUDIT_MD        TIMESTAMP,
  NAME            varchar(255),
  DESCRIPTION     varchar(255),
  STATUS          varchar(255),
  CATEGORY        varchar(255),
  REWARD_TYPE     varchar(255),
  REWARD_QUANTITY varchar(255),
  CREATOR_ID      BIGINT(19),
  SUBJECT_ID      BIGINT(19),
  END_DATE        TIMESTAMP    NULL,
  PERIOD_QUANTITY BIGINT(19)   NULL,
  PERIOD_UNIT     VARCHAR(255) NULL

);

create table FRIENDS_GROUP (
  GROUP_ID BIGINT(19) ,
    AUDIT_CD TIMESTAMP,
  AUDIT_RD TIMESTAMP,
  AUDIT_MD TIMESTAMP,
  NAME varchar(45)
);

create table POINT (
  POINT_ID BIGINT(19) auto_increment,
  QUANTITY BIGINT(19),
  POINT_TYPE varchar(45),
  USER_ID BIGINT(19),
  CHALLENGE_ID BIGINT(19),
  FOREIGN KEY (USER_ID) REFERENCES USER(USER_ID),
  FOREIGN KEY (CHALLENGE_ID) REFERENCES CHALLENGE(CHALLENGE_ID)
);

create table CHALLENGE_CATEGORY(
  CHALLENGE_CATEGORY_ID BIGINT(19) auto_increment,
  CHALLENGE_CATEGORY_NAME varchar(45)
);

CREATE TABLE NOTIFICATION (
  NOTIFICATION_ID BIGINT(19) AUTO_INCREMENT,
  AUDIT_CD        TIMESTAMP,
  AUDIT_RD        TIMESTAMP,
  AUDIT_MD        TIMESTAMP,
  TYPE            VARCHAR(256),
  STATUS          VARCHAR(256),
  MESSAGE         VARCHAR(256),
  CREATOR_ID      BIGINT(19),
  SUBJECT_ID      BIGINT(19),
  CHALLENGE_ID    BIGINT(19)
);
